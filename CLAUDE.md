...
- **加一个新源 = 在 `collect.py` 写个 adapter + 在 `sources.yml` 把 `adapter:` 从 null 改成它。**
  7 个源现在是 `adapter: null`（真实但没接），它们是最自然的下一批后端任务（见 `docs/SOURCES.md`）。
...
✅ 已搭好并实测跑通：
- `collect.py`：24 个源接通（generic_feed RSS/Atom + hf_daily_papers / hn_algolia / ossinsight /
  ...
  共用）/ alphaxiv，并把 karpathy 改指其 bearblog Atom 走 generic_feed）。fetch 与 parse 已分离
