...
- **加一个新源 = 在 `collect.py` 写个 adapter + 在 `sources.yml` 把 `adapter:` 从 null 改成它。**
  17 个源现在是 `adapter: null`（真实但没接），它们是最自然的下一批后端任务（见 `docs/SOURCES.md`）。
...
✅ 已搭好并实测跑通：
- `collect.py`：14 个源接通（generic_feed RSS/Atom + hf_daily_papers / hn_algolia / ossinsight /
  yc_launches），实测一次拉到 **3252 条** item，覆盖 14 源。带重试，单源失败不影响整体。
